...
import { bulkUpdateOrderProducts } from "@/lib/actions/order-actions";
...
export function BulkAssignDialog({ open, onOpenChange, orderId, orderName, productCount }: BulkAssignDialogProps) {
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await bulkUpdateOrderProducts(orderId, {
            ...
            if (result.error) {
                ...
                toast.success(`${productCount} ürün güncellendi`);
                onOpenChange(false);
                router.refresh();
            }
        } finally {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md">
                <DialogHeader>
                    <DialogTitle>Toplu Atama - {orderName}</DialogTitle>
                    <p className="text-sm text-slate-500">{productCount} ürüne uygulanacak</p>
                </DialogHeader>
